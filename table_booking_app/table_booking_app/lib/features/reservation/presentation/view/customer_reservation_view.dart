...
import 'package:table_booking_app/features/food_menu/presentation/state/food_menu_state.dart';
...
// call update reservation from viewmodel
  ...
  void _getDatePicker() {
    ...
    showDatePicker(
      ...
      lastDate: DateTime(
        2025,
      ), // allow to add today or tomorrow days but not past date
    ).then((pickedDate) {
      if (pickedDate == null) {
        return;
      }
...
//  function that allows to pick time from UI
  ...
  void _reservationSubmit() {
    if ((indoorSelected || outdoorSelected)) {
      ...
      String textMessage = 'Do you sure want to book table ?';
